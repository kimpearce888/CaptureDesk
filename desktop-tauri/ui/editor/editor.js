...
// ---- export-pipeline self test (CI: capturedesk --editor-smoke)

window.capturedesk.on('editor:smoke', async () => {
  try {
    console.info('[CaptureDesk] smoke: self test starting');
    const engine = await CDEditor.loadFFmpeg();
    console.info(`[CaptureDesk] smoke: engine loaded (${engine.engine} ${engine.version})`);
    // 1x1 red PNG — validates tmp write → demux → decode → native encode →
    // saved export, without the cost of a full video smoke on CI hardware.
    ...
    const inPath = `${tmp}/cd-smoke-in.png`;
    const staged = await window.capturedesk.writeFile(inPath, bytes);
    if (!staged || !staged.ok) throw new Error((staged && staged.error) || 'tmp write failed');
    const res = await window.capturedesk.transcodeFile({
      input: inPath,
      base: 'CaptureDesk_smoke',
      format: 'webm',
      trim: { inS: 0, outS: 0.04 },
      overlays: [],
      quality: 'source',
    });
    if (!res || !res.ok) throw new Error((res && res.error) || 'native transcode failed');
    console.info(`[CaptureDesk] smoke: native transcode output = ${res.file}`);
